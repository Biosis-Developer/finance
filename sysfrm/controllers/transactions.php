...
if($payerid == ''){
    $payerid = '0';
        ...
        if ($msg == '') {
            ...
            $d->save();
            $tid = $d->id();
            _log('Novo Deposito: '.$description.' [ID: '.$tid.' | Valor: '.$amount.']','Admin',$user['id']);
            _msglog('s',$_L['Transação Efetuada Com Sucesso']);
           echo $tid;
        } else {
           echo $msg;
        }
 ...
 ');
    ...
    case 'expense-post':
        ...
        if ($msg == '') {
            ...
            $d->save();
            $tid = $d->id();
            _log('Nova Despesa: '.$description.' [ID: '.$tid.' | Valor: '.$amount.']','Admin',$user['id']);
            _msglog('s',$_L['Transação Efetuada Com Sucesso']);
            echo $tid;
        } else {
            echo $msg;

...
if($d){
    echo 'Marcar atividades automáticas a partir de hoje: '.$today;

}
else{
    $syslogs = '';
    $syslogs .= '================================================== <br>';
    $syslogs .= date('Y-m-d H:i:s').' : Marcar Atividades Iniciadas....... <br>';
    ...
    if(($arcs['accounting_snapshot']) == 'Active'){
        $syslogs .= date('Y-m-d H:i:s').' : Abrindo Contabilidade <br>';
        ...
        $msg .= 'Total de Despesas: '.$config['currency_code'].' '.number_format($last_day_expense,2,$config['dec_point'],$config['thousands_sep']).'<br>';

        // echo $msg;
        $syslogs .= date('Y-m-d H:i:s').' : Contabilidade Criada! <br>';
        $syslogs .= '=============== Contabilidade ==================== <br>';
        $syslogs .= $msg;
        $syslogs .= '================================================== <br>';
...
//  ============================ For Invoice =======================================================================
    if(($arcs['recurring_invoice']) == 'Active'){
        $syslogs .= date('Y-m-d H:i:s').' : Criação de Fatura Recorrente <br>';
...
$c++;
        ...
        // echo $msg;
        $syslogs .= date('Y-m-d H:i:s').' : '.$c.' Fatura Criado! <br>';
    ...
    if(($arcs['notify']) == 'Active') {
        ...
        Notify_Email::_send($config['CompanyName'],$arcs['notifyemail'],$config['CompanyName'].' Automação de Atividade ',$syslogs);
    }
